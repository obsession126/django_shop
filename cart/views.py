...
class AddToCartView(CartMixin,View):
    ...
    def post(self,request,slug):
        ...
        if not form.is_valid():
            return JsonResponse({'error':'Invalud form data','errors':form.errors,},status=400)
        ...
        if existing_item:
            ...
            if total_quantity >product_size.stock:
                return JsonResponse({'error':f'Cannot add {quantity} items. Only{product_size.stock - existing_item.quantity} more available.'},status =400)
        ...
        if request.headers.get('HX-Request'):
            ...
            return JsonResponse({'success':True,'total_items':cart.total_items,'message':f"{product.name} added to cart",'cart_item_id':cart_item.id})
...
class CartCoutView(CartMixin,View):
    def get(self,request):
        cart = self.get_cart(request)
...
class ClearCartView(CartMixin,View):
    def post(self,request):
        ...
        return JsonResponse({'succes':True,'message':'Cart cleared'})
