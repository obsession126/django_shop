...
class OrderAdmin(admin.ModelAdmin):
    ...
    fieldsets = (
        ('Order Informaition',{'fields':('user','first_name','last_name','email','company','address1','address2','city','country','province','postal_code','phone','special_instructions','total_price')}),
        ('Payment and Status',{'fields':('status','payment_provider','stripe_payment_intend_id',)}),
        ('Timstamps',{'fields':('created_at','updated_at'),
                      'classes':('collapse',)}),
    )


def get_readonlyfields(self,request,obj=None):
    if obj:
